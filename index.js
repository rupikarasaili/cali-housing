...
// ---- Scaler params copied from  notebook ----
...
// Build inputs ONCE and prefill with defaults
function buildInputs() {
  ...
  FEATURE_NAMES.forEach((name, i) => {
    ...
    // ⬇️ hard-code default values so they appear immediately
    input.value = DEFAULT_ROW[i] ?? '';

    // Helps mobile keyboards accept decimals
...
// (Optional) if you have a "Load sample row" button with id="loadDefault"
const loadBtn = document.getElementById('loadDefault');
if (loadBtn) {
  loadBtn.addEventListener('click', (e) => {
    e.preventDefault();
    FEATURE_NAMES.forEach((name, i) => {
      const el = document.getElementById(`f_${name}`);
      if (el) el.value = DEFAULT_ROW[i] ?? '';
    });
  });
}
...
async function predict(e) {
  ...
  try {
    ...
    // input/output names must match your torch.onnx.export
    const outputs = await session.run({ input: inputTensor });
    const yScaled = outputs.output.data[0]; // if your node is named "output"
    const y = inverseY(yScaled);            // unscale
